...
test('reference v2 is rendered above every v15.0 optical size', async ({ page }) => {
  ...
  const columns = sizes.map(size => {
    ...
    return `<section class="column"><h2>${size} px</h2><div class="tile"><img data-kind="reference" width="${size}" height="${size}" src="${REFERENCE_DATA_URL}"></div><div class="caption">REFERENCE V2</div><div class="tile"><img data-kind="candidate" width="${size}" height="${size}" src="${candidate}"></div><div class="caption">CURRENT v15.0</div></section>`;
  }).join('');
  await page.setContent(`<style>${comparisonCss}</style><main><header><div><h1>CANONICAL REFERENCE V2 / CURRENT v15.0</h1><p>Judge macro scale first, then the high layered hood, broad irregular cavern, gathered crushed cowl, three broken fold families, upper-side aura, clean lower edge and optical readability.</p></div><div class="decision">v15.0: NOT REFERENCE APPROVED</div></header><div class="matrix">${columns}</div><footer>Green rendering and layered movement prove technical integrity only. Visual fidelity remains a manual decision.</footer></main>`);
...
test('reference v2 is compared with v15.0 SVG and exact checked homepage build', async ({ page }) => {
  ...
  await page.setContent(`<style>*{box-sizing:border-box}html,body{margin:0;background:#03070d;color:#d9f8ff;font:14px system-ui}main{padding:28px}h1{margin:0 0 8px;font-size:23px}.sub{color:#8ebcca;margin-bottom:22px}.top{display:grid;grid-template-columns:1fr 1fr;gap:18px}figure{margin:0;display:grid;gap:8px}.panel{height:340px;background:#010306;border:1px solid rgba(101,218,244,.18);display:grid;place-items:center;overflow:hidden}.panel img{display:block;max-width:100%;max-height:100%;object-fit:contain}.live{margin-top:18px}.live .panel{height:320px;background:#050810}.live img{width:100%;height:100%;object-fit:contain}figcaption{font-size:12px;letter-spacing:.12em;color:#88becb}.decision{float:right;color:#ffb27a;font-weight:700}</style><main><h1>REFERENCE V2 / v15.0 SVG / EXACT CHECKED BUILD <span class="decision">NOT REFERENCE APPROVED</span></h1><div class="sub">Layered pointer depth is tested separately; the lower edge remains dark and smoke-free.</div><div class="top"><figure><div class="panel"><img src="${REFERENCE_DATA_URL}" width="300" height="300"></div><figcaption>CANONICAL REFERENCE V2</figcaption></figure><figure><div class="panel"><img src="${BASE_URL}/brand-emblem.svg?v=${VERSION}" width="300" height="300"></div><figcaption>CURRENT v15.0 SVG</figcaption></figure></div><figure class="live"><div class="panel"><img src="${liveUrl}"></div><figcaption>CHECKED HOMEPAGE HEADER</figcaption></figure></main>`);
