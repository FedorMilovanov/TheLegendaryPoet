...
test('reference v2 is rendered above every v11.5 optical size', async ({ page }) => {
  ...
  const columns = sizes.map(size => {
    ...
    return `<section class="column"><h2>${size} px</h2><figure><div class="tile"><img data-kind="reference" data-size="${size}" src="${REFERENCE_DATA_URL}" width="${size}" height="${size}"></div><figcaption>REFERENCE V2</figcaption></figure><figure><div class="tile"><img data-kind="candidate" data-size="${size}" src="${candidate}" width="${size}" height="${size}"></div><figcaption>CURRENT v11.5</figcaption></figure></section>`;
  }).join('');
  await page.setContent(`<style>${css}</style><main><header><div><h1>CANONICAL REFERENCE V2 / CURRENT v11.5</h1><p>Judge macro silhouette, hood and face proportions, crushed cowl, non-radial fold masses, upper-side electrical aura and the clean smoke-free lower edge.</p></div><div class="decision">v11.5: NOT REFERENCE APPROVED</div></header><div class="matrix">${columns}</div><footer>Exact main head. Green rendering proves technical integrity only.</footer></main>`);
...
test('reference v2 is compared with v11.5 SVG and exact-main live homepage', async ({ page }) => {
  ...
  await page.setContent(`<style>*{box-sizing:border-box}html,body{margin:0;background:#03070d;color:#d9f8ff;font:14px system-ui}main{padding:28px}h1{margin:0 0 8px;font-size:23px}.sub{color:#8ebcca;margin-bottom:22px}.top{display:grid;grid-template-columns:1fr 1fr;gap:18px}figure{margin:0;display:grid;gap:8px}.panel{height:340px;background:#010306;border:1px solid rgba(101,218,244,.18);display:grid;place-items:center;overflow:hidden}.panel img{display:block;object-fit:contain;max-width:100%;max-height:100%}.live{margin-top:18px}.live .panel{height:320px;background:#050810}.live img{width:100%;height:100%;object-fit:contain}figcaption{font-size:12px;letter-spacing:.12em;color:#88becb}.decision{color:#ffb27a;font-weight:700;float:right}</style><main><h1>REFERENCE V2 / v11.5 SVG / EXACT-MAIN LIVE SITE <span class="decision">NOT REFERENCE APPROVED</span></h1><div class="sub">The lower edge must remain dark and smoke-free.</div><div class="top"><figure><div class="panel"><img src="${REFERENCE_DATA_URL}" width="300" height="300"></div><figcaption>CANONICAL REFERENCE V2</figcaption></figure><figure><div class="panel"><img src="${BASE_URL}/brand-emblem.svg?v=${VERSION}" width="300" height="300"></div><figcaption>CURRENT v11.5 SVG</figcaption></figure></div><figure class="live"><div class="panel"><img src="${liveUrl}"></div><figcaption>EXACT-MAIN LIVE HOMEPAGE HEADER</figcaption></figure></main>`);
