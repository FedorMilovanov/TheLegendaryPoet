...
async function measure(page, request, source) {
  ...
  await expect(root).toHaveAttribute('data-brand-reference-decision', 'not-reference-approved');
...
test('v20 full-size and independent micro masters pass numeric geometry without receiving visual approval', async ({ page, request }) => {
  ...
  fs.writeFileSync(
    path.join(DIR, 'brand-v20-contract-metrics.json'),
    `${JSON.stringify({
...
test('v20 canonical comparison and optical-size evidence are rendered from exact committed assets', async ({ page }) => {
  ...
  const fullUrl = `${BASE_URL}/${path.basename(sheet.candidates[0].file)}`;
  const microUrl = `${BASE_URL}/${path.basename(sheet.candidates[1].file)}`;
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          body { margin: 0; background: #05080c; color: #d9f8ff; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
          main { padding: 32px; }
          h1 { margin: 0 0 8px; font-size: 25px; letter-spacing: .06em; }
          .status { margin-bottom: 26px; color: #7fdff3; font-size: 15px; }
          .grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 22px; }
          .card { min-height: 430px; padding: 18px; border: 1px solid #173440; background: #020406; }
          .label { margin-bottom: 14px; color: #a9ecf8; font-weight: 700; }
          .stage { height: 352px; display: grid; place-items: center; position: relative; overflow: hidden; background: #010204; }
          .stage img { width: 320px; height: 320px; object-fit: contain; }
          .overlay img { position: absolute; inset: 16px; width: 320px; height: 320px; }
          .overlay .candidate { opacity: .58; }
          .sizes { margin-top: 24px; display: flex; align-items: flex-end; gap: 24px; padding: 20px; border: 1px solid #173440; background: #020406; }
          .sample { min-width: 132px; text-align: center; }
          .sample b { display: block; margin-bottom: 12px; color: #78ddef; }
          .sample .well { height: 280px; display: flex; align-items: flex-start; justify-content: center; background: #010204; padding-top: 20px; }
          .sample img { display: block; }
        </style>
      </head>
      <body>
        <main>
          <h1>CANONICAL REFERENCE / v20.3 / LANDMARK OVERLAY</h1>
          ...
          <section class="grid">
            ...
            <article class="card"><div class="label">v20.3 FULL MASTER 256×256</div><div class="stage"><img src="${fullUrl}"></div></article>
          ...
          <section class="sizes">
            ${[64, 96, 128, 256].map((size) => `<div class="sample"><b>${size}px</b><div class="well"><img src="${fullUrl}" width="${size}" height="${size}"></div></div>`).join('')}
          </section>
        </main>
      </body>
    </html>`);
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          p { margin: 0 0 24px; color: #7fdff3; }
          .rows { display: grid; gap: 22px; }
          .row { display: grid; grid-template-columns: 150px repeat(5, 1fr); gap: 16px; align-items: stretch; }
          .row-title { display: grid; place-items: center; border: 1px solid #173440; font-weight: 700; }
          .cell { height: 320px; border: 1px solid #173440; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding: 16px; }
          .dark { background: #010204; }
          .light { background: #e9f1f4; color: #061016; }
          .cell b { margin-bottom: 18px; }
          .actual { height: 74px; display: grid; place-items: start center; width: 100%; }
          .magnified { margin-top: 18px; width: 160px; height: 160px; display: grid; place-items: center; border: 1px dashed currentColor; opacity: .94; }
          .magnified img { width: 144px; height: 144px; }
        </style>
      </head>
      <body>
        <main>
          <h1>v20.1 INDEPENDENT MICRO MASTER — DARK / LIGHT DIAGNOSTICS</h1>
          <p>Actual optical render above; enlarged structural inspection below. not-reference-approved.</p>
          <div class="rows">
            ${['dark', 'light'].map((tone) => `
              <section class="row">
                <div class="row-title ${tone}">${tone.toUpperCase()}</div>
                ${[16, 20, 24, 32, 48].map((size) => `
                  <div class="cell ${tone}">
                    <b>${size}px</b>
                    <div class="actual"><img src="${microUrl}" width="${size}" height="${size}"></div>
                    <div class="magnified"><img src="${microUrl}"></div>
                  </div>`).join('')}
              </section>`).join('')}
          </div>
        </main>
