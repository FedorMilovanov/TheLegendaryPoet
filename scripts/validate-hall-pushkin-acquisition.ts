...
for (const required of [contractPath, rightsPath, slicePath, acquisitionPath, ownerDispositionPath, validatorPath, byteEvidenceScriptPath, byteEvidenceWorkflowPath]) {
...
expect(contract.phase === 'pushkinVerticalSlice' && same(contract.gates, EXPECTED_GATES), 'offline source authoring may not advance or reopen Hall gates');
expect(contract.productionRoute?.mode === 'placeholder' && contract.productionRoute?.allowThreeRuntimeImports === false, 'offline source authoring may not activate production Hall WebGL');
...
expect(contract.sourceAuthority?.pushkinOwnerDisposition === ownerDispositionPath, 'Hall source authority must bind owner offline-authoring disposition');
...
expect(acquisition.phase === 'pushkinVerticalSlice' && acquisition.status === 'source-bytes-verified-offline-authoring-authorized-production-rights-pending', 'acquisition status must expose exact-source offline authoring while production rights remain pending');
...
expect(acquisition.rules?.offlineAuthoringMayUseOwnerAuthorizedExactVerifiedSources === true && acquisition.rules?.offlineAuthoringIsProductionPermission === false, 'offline authoring must be explicit and must not become production permission');
expect(ownerDisposition.status === 'owner-offline-authoring-authorized' && ownerDisposition.offlineAuthoring?.authorized === true, 'owner disposition must authorize the offline source authoring lane');
expect(ownerDisposition.offlineAuthoring?.productionShippingAuthorizedByThisDecision === false, 'owner disposition may not authorize production shipping');
...
expect(portrait?.objectProvenance?.status === 'source-verified' && portrait?.rightsStatus === 'rights-pending' && portrait?.verificationStatus === 'rights-pending', 'Kiprensky object must be verified while production rights remain pending');
expect(portraitAcq?.acquisitionState === 'source-bytes-verified-offline-authoring-authorized-production-rights-pending' && portrait?.reproduction?.acquisitionStatus === 'source-bytes-verified-rights-pending', 'Kiprensky acquisition must distinguish owner-authorized offline use from canonical production rights');
expect(portraitAcq?.offlineAuthoringAuthorized === true, 'Kiprensky exact source must be owner-authorized for offline authoring');
...
expect(portrait?.creditLine == null && portrait?.runtimeAssetPath == null && portrait?.productionManifestEligible === false, 'Kiprensky offline authorization may not imply final production approval');
...
expect(onegin?.objectProvenance?.status === 'source-verified' && onegin?.rightsStatus === 'rights-pending' && onegin?.verificationStatus === 'rights-pending', '1833 Onegin object must be verified while production rights remain pending');
expect(oneginAcq?.acquisitionState === 'source-bytes-verified-offline-authoring-authorized-production-rights-pending' && onegin?.reproduction?.acquisitionStatus === 'source-bytes-verified-rights-pending', '1833 Onegin acquisition must distinguish owner-authorized offline use from canonical production rights');
expect(oneginAcq?.offlineAuthoringAuthorized === true && oneginAcq?.selectedOfflineSourcePdfPageIndex === 0, '1833 Onegin title page must be owner-authorized for offline authoring');
...
expect(onegin?.creditLine == null && onegin?.runtimeAssetPath == null && onegin?.productionManifestEligible === false, 'Onegin offline authorization may not imply final production approval');
...
expect(pushkinHouseAcq?.acquisitionState === 'institutional-copy-request-required-optional-for-v1' && pushkinHouseAcq?.originalFileUrl == null && pushkinHouseAcq?.bytesAcquired === false && pushkinHouseAcq?.sourceFileHash == null, 'Pushkin House copy must remain unacquired and optional for v1');
expect(pushkinHouseAcq?.institutionalRequest?.requiredForExactArchiveCopy === true && pushkinHouseAcq?.institutionalRequest?.status === 'not-submitted', 'gratis inquiry must not be misrepresented as formal archive copy order');
expect(pushkinHouseAcq?.gratisGuidanceInquiry?.status === 'sent' && pushkinHouseAcq?.gratisGuidanceInquiry?.isFormalPaidCopyOrder === false, 'actual gratis guidance inquiry must be recorded without claiming a paid copy order');
expect(pushkinHouseAcq?.v1Requirement === 'optional-deferred-not-blocking' && pushkinHouseAcq?.offlineAuthoringAuthorized === false, 'Pushkin House must not block or enter v1 offline exhibit without reusable bytes');
...
expect(weakAcq?.acquisitionState === 'do-not-acquire-current-weak-source' && weakAcq?.originalFileUrl == null && weakAcq?.bytesAcquired === false && weakAcq?.sourceFileHash == null && weakAcq?.offlineAuthoringAuthorized === false, 'weak mirror must remain do-not-acquire/unhashed/unusable');

const provenanceCandidate = (slice.exhibit?.provenanceCandidates ?? []).find((entry: any) => entry.role === 'documentaryDrawing');
expect(provenanceCandidate?.preferredAssetId === 'pushkin-house-onegin-self-portrait-1824', 'Pushkin slice must retain official Pushkin House manuscript provenance candidate');
...
expect(provenanceCandidate?.v1Requirement === 'optional-deferred-not-blocking', 'Pushkin House must be optional/deferred for v1');
...
expect(slice.productionBoundary?.offlineSourceEvidenceMediaAllowed === true, 'slice must allow exact-source offline evidence media');
expect(slice.productionBoundary?.productionManifestAllowed === false && slice.productionBoundary?.productionThreeRuntimeMayActivate === false, 'offline authoring may not unblock production manifest or Three runtime');

expect(acquisition.currentOutcome?.approvedDocumentaryAssets === 0, 'approved production documentary assets must remain zero');
expect(acquisition.currentOutcome?.exactSourceByteHashes === 2, 'exact acquired-byte hash count must be two');
expect(acquisition.currentOutcome?.offlineBlenderSourceEvidenceAllowed === true && same(acquisition.currentOutcome?.offlineAuthorizedAssetIds, EXPECTED_OFFLINE_IDS), 'offline Blender source-evidence authorization must be exact and bounded');
expect(acquisition.currentOutcome?.productionManifestAllowed === false && acquisition.currentOutcome?.blenderExhibitMayConsumeDocumentaryMedia === false && acquisition.currentOutcome?.productionWebglMayBegin === false, 'offline evidence authoring may not unlock production documentary consumption/runtime');
for (const allowed of ['independent-byte-revalidation-against-recorded-sha256','author-owner-authorized-exact-source-offline-pushkin-exhibit','derive-and-hash-offline-display-assets','measure-first-slice-delivery-and-performance','produce-required-offline-visual-evidence','final-credit-disposition','promote-only-fully-approved-production-records']) expect((acquisition.nextBoundary?.allowed ?? []).includes(allowed), `acquisition next boundary missing ${allowed}`);
...
console.log('Hall Pushkin acquisition authority passed: exact verified portrait/Onegin bytes are owner-authorized for bounded offline Blender evidence, while production rights, manifest, WebGL and optional Pushkin House copy remain fail-closed.');
