...
const publicBlocks: EssayBlock[] = source.blocks.map((block) => {
  const { id: _internalEvidenceId, ...publicBlock } = block as EssayBlock & { id?: string };
  return publicBlock as EssayBlock;
});
if (publicBlocks.some((block) => 'id' in block)) {
  throw new Error('internal evidence ids remain in public blocks');
}
...
type ExtendedEssaySource = EssaySource & {
  aliases?: string[];
  kind?: EssaySource['kind'] | 'institutional';
};
...
const sources = (source.sources ?? []).map((record): EssaySource => {
  ...
  const extended = record as ExtendedEssaySource;
  const { aliases: _internalAliases, ...withoutAliases } = extended;
  const normalizedKind = extended.kind === 'institutional' ? 'context' : extended.kind;
  const normalized: EssaySource = {
    ...withoutAliases,
    kind: normalizedKind,
  } as EssaySource;
...
if (sources.some((record) => 'aliases' in record || String(record.kind) === 'institutional')) {
  throw new Error('internal aliases or institutional source kind remain in public bibliography');
}
