...
for(const t of ['useId()','useReducedMotion()','requestAnimationFrame(','cancelAnimationFrame(','data-brand-interaction="idle"','data-brand-parallax="layered-v1"',"pointerType!=='touch'","pointerEvents:'none'","import rawVector from './brandEmblemV17.svg?raw'","const BRAND_VERSION='cloak-20260729-20'"])assert.ok(component.includes(t),`component missing ${t}`);
assert.match(component,/const\s+VECTOR_SOURCE\s*=\s*['"]canonical-reference-v2-black-monolith-v17-0['"]/,'component VECTOR_SOURCE differs from canonical v17 source');
