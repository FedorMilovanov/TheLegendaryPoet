...
assert.match(component, /const cloakPath = 'M18 43C10 46/, 'runtime cloak must retain the broad reference-derived silhouette');
assert.match(component, /const hoodPath = 'M48 2\.5C40 5\.4/, 'runtime hood must retain the lower, wider shell');
assert.match(component, /const voidPath = 'M48\.1 15\.3C39\.8 15\.3/, 'runtime void must retain the rounded cavern opening');
...
assert.match(standaloneSvg, /M18 43C10 46/, 'standalone cloak no longer follows the broad silhouette');
assert.match(standaloneSvg, /M48 2\.5C40 5\.4/, 'standalone hood no longer follows the refined shell');
assert.match(standaloneSvg, /M48\.1 15\.3C39\.8 15\.3/, 'standalone face void changed');
...
assert.match(microSvg, /M6 14C3\.3 15/, 'micro cloak must retain the broad shoulder silhouette');
assert.match(microSvg, /M16 \.8C13\.3 1\.8/, 'micro hood geometry changed');
assert.match(microSvg, /M16\.1 5C13\.3 5/, 'micro void geometry changed');
...
assert.match(maskSvg, /M48 2\.5C40 5\.4/, 'Safari mask must preserve the hooded outer silhouette');
assert.match(maskSvg, /M48\.1 15\.3C39\.8 15\.3/, 'Safari mask must preserve the face opening');
...
console.log('brand validation: v8.4 broad mantle, deep hood, cavern void, asymmetrical wrap and valid XML assets are consistent');
