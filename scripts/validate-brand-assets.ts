...
const vectorSource = 'reference-derived-contours-v8-20';
...
assert.match(component, /const VECTOR_SOURCE = 'reference-derived-contours-v8-20'/, 'v8.20 contour provenance constant is missing');
assert.match(component, /data-brand-vector-source=\{VECTOR_SOURCE\}/, 'BrandMark must expose the v8.20 vector source');
...
assert.match(component, /const voidPath = 'M47\.5 17\.8C40\.3 18\.2/, 'runtime face void geometry changed');
...
assert.match(standaloneSvg, /M47\.5 17\.8C40\.3 18\.2/, 'standalone face void changed');
...
assert.match(microSvg, /M15\.9 5\.7C13\.7 5\.8/, 'micro face void geometry changed');
...
assert.match(maskSvg, /M47\.5 17\.8C40\.3 18\.2/, 'Safari mask must preserve the face opening');
...
console.log('brand validation: v8.20 asymmetric textile hood, cavernous void, wrapped cowl, restrained broken rim and platform fallbacks are consistent');
