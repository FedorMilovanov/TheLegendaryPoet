...
assert.ok((candidate.match(/data-brand-aura-(?:haze|branches)/g) ?? []).length === 2, 'aura haze/branch separation is missing');

assert.equal(ledger.marathonId, 'square-closeup-reference-v19');
assert.match(ledger.latestCandidate, /v19\.11 reference-led irregular-aura refinement/i);
...
assert.equal(evaluation.passHistory.length, 11);
assert.deepEqual(evaluation.passHistory.map((item) => item.pass), ['v19.1', 'v19.2', 'v19.3', 'v19.4', 'v19.5', 'v19.6', 'v19.7', 'v19.8', 'v19.9', 'v19.10', 'v19.11']);
...
assert.ok(evaluation.nextPasses.some((item) => item.startsWith('v19.12')));
...
assert.doesNotMatch(production, /v19\.11-reference-geometry-reset/, 'unreviewed candidate leaked into production SVG');
assert.doesNotMatch(component, /brand-emblem-v19-candidate/, 'unreviewed candidate leaked into BrandMark');

console.log('brand candidate validation: eleven reference-led passes are locked; v19.11 remains isolated from production');
