...
expect(commandSource.includes('onKeyDown={onInputKeyDown}'), 'command Arrow/Enter navigation must be owned by the focused combobox input');
expect(!commandSource.includes('onKeyDown={onDialogKeyDown}'), 'command dialog must not intercept Enter from focused result or close buttons');
expect(commandSource.includes('React.KeyboardEvent<HTMLInputElement>'), 'command navigation handler must remain input-scoped at the type boundary');
...
console.log('Interaction runtime validation passed: centralized Escape, route focus, focused command keyboard ownership, rating draft safety, resilient images and bounded pointer effects.');
