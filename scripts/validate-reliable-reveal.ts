...
assert.match(helpers, /window\.scrollTo\(\{ top: scrollTop, left: 0, behavior: 'auto' \}\)/);
assert.match(helpers, /actualTop = document\.scrollingElement\?\.scrollTop \?\? window\.scrollY/);
assert.match(helpers, /currentScroll = scrollingElement\?\.scrollTop \?\? window\.scrollY/);
assert.match(helpers, /viewportTop = visualViewport\?\.offsetTop \?\? 0/);
assert.match(helpers, /desiredRectTop = viewportTop \+ Math\.max\(24, \(viewportHeight - visibleHeight\) \/ 2\)/);
assert.match(helpers, /WEBKIT_VIEWPORT_STABILITY_MS = 1_200/);
assert.match(helpers, /await page\.waitForTimeout\(WEBKIT_VIEWPORT_STABILITY_MS\)/);
assert.match(helpers, /const second = await readViewportState\(target\)/);
assert.match(helpers, /Math\.abs\(second\.centerDelta\) <= Math\.max\(48, second\.viewportHeight \* 0\.3\)/);
assert.match(helpers, /second\.intersects && centered/);
...
console.log('reliable reveal validation: real viewport geometry drives measurable CSS transitions while Safari QA uses one scroll coordinate system, visualViewport-aware targets and two-sample stable centering without forced visual mutation');
