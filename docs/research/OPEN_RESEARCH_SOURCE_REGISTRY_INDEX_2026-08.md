...
**Current verification pass:** `AUDIT-2026-08-03-P09`  
...
The 51 directly audited rows currently comprise:
...
These failures are not promoted to verified and are not automatically treated as permanent death.

### Broken rows removed from active maps

- `OSR-0282` — stored Gallica *Dom Juan* selection returned 404.
- `OSR-0286` — stored Gallica Symbolism selection returned 404.

Both remain in audit history with `DROP-BROKEN`.
...
## 5. Verified binaries and duplicate controls
...
## 6. Controlling policy and reports
...
- `docs/research/OPEN_RESEARCH_SOURCE_AUDIT_AND_CLEANUP_PASS_09_2026-08-03.md` — first direct audit and cleanup wave;
...
## 7. Next direct-audit order

No new-source discovery is authorised while the existing registry remains mostly unaudited.

Audit the 236 pending rows by existing source cluster:

1. ФЭБ and РВБ academic corpora;
2. НЭБ, РГБ, dlib and book-monument records;
3. museums, РГАЛИ and archive finding aids;
4. university repositories and theses;
5. Wikisource, Culture.ru and commercial/popular navigation;
6. the legacy quotations and low-authority links in `docs/RESEARCH_SOURCES.md`.
