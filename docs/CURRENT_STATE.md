...
11. **Hall v3 staged boundary.** Production `/hall` remains a lightweight DOM placeholder; retained Hall-v2 source is forensic/non-authoritative and excluded from current TypeScript/runtime ownership. Foundation and Reference Bible are completed and persistently guarded. The active Hall phase is metric-greybox tooling/preflight: the repository pins a checksum-verified Blender runtime and equal H1/H2/H3 comparison contract while no candidate, topology or camera rig is approved.

## Current quality gates

The `Project contracts` workflow runs project-contract, workflow-consolidation, Hall phase contracts, reader-certification and UTC-day validators on every PR and `main` push. `npm run check` validates content, brand, app shell, interaction runtime, Hall foundation invariants, completed Reference Bible evidence, metric-greybox tooling contracts, routes, archive behavior and TypeScript. A separate path-scoped Hall greybox workflow proves the pinned Blender runtime through a headless metric save/reopen smoke scene; normal production build, prerender and browser workflows remain separate final gates.
...
- `TLP-HALL-001` — Product #369 owns the staged Hall v3 rebuild. Foundation and Reference Bible are merged/completed. The current source phase is **metricGreybox**, limited first to reproducible Blender tooling and candidate-equality preflight. `docs/hall-v3/greybox-tooling.json` pins the active DCC witness; `greybox-candidates.json` keeps H1/H2/H3 `unbuilt` and all decisions unset. Production `/hall` remains a lightweight DOM shell; camera approval, material/light/export, Pushkin slice and all web-runtime gates remain blocked.
