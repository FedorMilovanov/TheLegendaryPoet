...
11. **Hall v3 staged boundary.** Production `/hall` remains a lightweight DOM placeholder and Hall v2 remains forensic/non-authoritative. Foundation, Reference Bible, metric greybox, Camera Approval and the material/light/export gate are completed. H3 is frozen topology authority and R1 is the approved guided camera. The selected Pushkin baseline is L0 minimal runtime lighting with metre-scaled UV0 surface mapping; UV1 remains optional reserve, the current L1 lightmap bake is rejected, and final production texture encoding is still deferred. `pushkinVerticalSlice` is the only active Hall gate. Its rights/source authority separates documentary object provenance, digital-reproduction rights and actual byte acquisition. The exact Commons originals for the Kiprensky portrait and 1833 `Eugene Onegin` are independently byte-verified and bound to exact SHA-256 identities. A separate exact-reproduction evidence review is now complete without changing canonical rights: the Kiprensky reproduction has strong public-domain copyright evidence but still requires an owner/legal decision for the applicable Tretyakov/Russian museum-use context, while the Onegin PDF has strong public-domain evidence but still requires owner production/credit disposition before it can become a finished Hall asset. The stronger Pushkin House manuscript object remains source-verified and requires an institutional copy request. Approved documentary assets remain zero; offline visual approval, documentary Blender media consumption, production WebGL runtime, web approval and scale-out remain blocked.

## Current quality gates

`Project contracts` runs project/workflow contracts, permanent Hall boundaries, reader certification and UTC-day validation on every PR and `main` push. `npm run check` keeps Hall foundation and Reference Bible guards, frozen topology provenance, persistent post-material H3/R1/L0 authority, the Pushkin rights/source-slice semantic validator, the canonical `RIGHTS_REGISTER` schema validator, the acquisition/byte-identity validator and the phase-independent material transport guard. The path-scoped Hall Blender workflow triggers on rights/slice/acquisition authority changes, validates all three Pushkin source layers before Blender download, then still regenerates frozen H1/H2/H3 and R0/R1/R2/R3 evidence, rebuilds the representative material bay, validates raw/optimized GLB transport and re-runs the selected/rejected browser witnesses under the active Pushkin gate. The separate Pushkin source-byte evidence workflow downloads only the two registered Commons originals into ephemeral runner storage, verifies file identity and recomputes SHA-256 against canonical hashes while uploading only JSON evidence. [`hall-v3/pushkin-rights-review.json`](hall-v3/pushkin-rights-review.json) and [`hall-v3/PUSHKIN_RIGHTS_REVIEW.md`](hall-v3/PUSHKIN_RIGHTS_REVIEW.md) are additive evidence records only: they may narrow the known human boundary but cannot convert `rights-pending` into `approved`. Earlier camera/material authoring and decision validators remain available for forensic reproduction but are no longer the current-phase authority.
...
- `TLP-HALL-001` — Product #369 owns the staged Hall v3 rebuild. The current machine phase is **pushkinVerticalSlice**. H3 topology, R1 camera and the selected L0/UV0 material-delivery contract are frozen; UV1 remains optional reserve, the current L1 bake remains rejected and production texture encoding remains deferred until measured inside the slice. `pushkin-rights.json`, `pushkin-slice.json` and `pushkin-acquisition.json` distinguish object identity, reproduction/intended-use disposition and actual-byte acquisition. The Kiprensky 1827 portrait and 1833 `Eugene Onegin` publication remain canonically `rights-pending`, with exact byte identities fixed as portrait `sha256:316d5f366a46f23cd0a181e570f2d09a6b0d12bc368dab18fdb394b8b8b8bf4b` and Onegin `sha256:d629c10943cbf6428eabb194ee5c17c1b763c27108a2238eaf72fadb275643e5`. The additive exact-reproduction review has reached the autonomous boundary: Kiprensky requires an owner/legal decision about intended commercial/non-commercial and Russia-facing museum-use treatment before canonical approval; Onegin has strong copyright/public-domain evidence for the exact PDF but still requires owner acceptance of intended use, exact documentary presentation/derivative and final credit. The official Pushkin House record `Ф. 244, оп. 12, ед. хр. 6` remains the preferred manuscript object-provenance candidate, but its digital copy requires a human/institutional request that remains `not-submitted`. The old weak mirror remains `blocked` as a negative control. Approved documentary assets = 0, exact source byte hashes = 2, runtime documentary paths = 0, and the production Hall manifest remains blocked. No further autonomous evidence gathering should manufacture approval from the existing record; the next material change requires owner/legal/institutional disposition or materially new evidence. This phase may not replace `/hall`, activate production Three/R3F/WebGL, consume rights-pending documentary media in the Blender exhibit, redesign H3/R1, start full-Hall lookdev or advance `offlineVisualApproval`, `webVerticalSlice` or `fullMuseumScaleOut`.
