...
11. **Hall v3 staged boundary.** Production `/hall` remains a lightweight DOM placeholder; retained Hall-v2 source is forensic/non-authoritative and excluded from current TypeScript/runtime ownership. Foundation and Reference Bible are completed and persistently guarded. Metric greybox is active: the checksum-verified Blender runtime is proven, and the current source defines three materially distinct H1/H2/H3 layouts generated by one equal-quality Blender pipeline. No candidate, topology or camera rig is approved.

## Current quality gates

The `Project contracts` workflow runs project-contract, workflow-consolidation, Hall phase contracts, reader-certification and UTC-day validators on every PR and `main` push. `npm run check` validates content, brand, app shell, interaction runtime, Hall foundation invariants, completed Reference Bible evidence, metric-greybox tooling and neutral-candidate source contracts, routes, archive behavior and TypeScript. The path-scoped Hall greybox workflow reuses one pinned Blender download to prove the tooling smoke scene and generate H1/H2/H3 comparison artifacts; generated evidence is runtime-validated before upload. Normal production build, prerender and browser workflows remain separate final gates.
...
- `TLP-HALL-001` — Product #369 owns the staged Hall v3 rebuild. Foundation and Reference Bible are completed. The current phase is **metricGreybox**, now in neutral H1/H2/H3 candidate authoring. `greybox-tooling.json` pins the proven DCC witness; `greybox-layouts.json` defines the auditable metre-scale layouts; `greybox-candidates.json` keeps `approvedCandidate=null` and `approvedRig=null` while one common 35 mm test lens is used only for equal comparison. Generated `.blend`/PNG/SVG/JSON are Actions evidence, not production assets. Camera approval, material/light/export, Pushkin slice and all web-runtime gates remain blocked.
