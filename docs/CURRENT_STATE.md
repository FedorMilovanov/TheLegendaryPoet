...
11. **Hall v3 staged boundary.** Production `/hall` remains a lightweight DOM placeholder; retained Hall-v2 source is forensic/non-authoritative and excluded from current TypeScript/runtime ownership. Foundation, Reference Bible and metric greybox are completed. The frozen neutral shootout selected H3 as the single topology authority for camera evaluation, retains H1 as reserve and rejects H2. Camera Approval is active, but no camera rig is approved; the common 35 mm lens remains benchmark-only. Materials, lighting/export, Pushkin slice and all web-runtime gates remain blocked.

## Current quality gates

The `Project contracts` workflow runs project-contract, workflow-consolidation, Hall phase contracts, reader-certification and UTC-day validators on every PR and `main` push. `npm run check` validates content, brand, app shell, interaction runtime, Hall foundation invariants, completed Reference Bible evidence, persistent topology-selection authority, routes, archive behavior and TypeScript. The path-scoped Hall greybox workflow reuses one pinned Blender download to prove the tooling smoke scene and regenerate the frozen H1/H2/H3 comparison evidence; the persistent selection validator requires that regenerated route metrics, layout fingerprints, clearances and sightlines still match the evidence used to select H3. Normal production build, prerender and browser workflows remain separate final gates.
...
- `TLP-HALL-001` — Product #369 owns the staged Hall v3 rebuild. Foundation, Reference Bible and metric greybox are completed. H3 is the selected topology; H1 is a retained reserve benchmark; H2 is rejected for the current production path. The current phase is **cameraApproval**. `greybox-decision.json` binds that decision to exact PR #376 Blender evidence and frozen layout fingerprints. `approvedRig=null`; the prior 35 mm comparison lens is benchmark-only and must not be mistaken for Camera Approval. The next bounded wave must prove guided H3 desktop/mobile/reduced-motion framing without redesigning the topology. Material/light/export, Pushkin slice, offline visual approval, web runtime and scale-out remain blocked.
