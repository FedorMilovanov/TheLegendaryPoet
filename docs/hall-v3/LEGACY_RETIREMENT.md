...
`src/components/hall/*` is retained temporarily as forensic/technical evidence from Hall v2. It is **not** current visual authority and is not imported by production `/hall`.
...
`scripts/validate-hall-audio-runtime.ts` is a Hall-v2 validator. It must stop running inside the mandatory current `validate:interaction-runtime` chain once `scripts/validate-hall-foundation.ts` protects the dormant-route/legacy-isolation boundary.

The old file may remain temporarily for forensic reference, but green output from it is not Hall-v3 evidence.
