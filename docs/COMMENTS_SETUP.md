...
После полного подключения shared mode локальный v3 outbox остаётся атомарным. Если сеть, Worker, D1 или Turnstile временно недоступны уже после активации, запись не объявляется синхронизированной: она остаётся в очереди и повторяется позже. Публичный корпус комментариев при этом не копируется целиком в localStorage.
