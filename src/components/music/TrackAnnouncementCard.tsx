    ...
    >
      {track.wideCoverUrl && (
        <img
          ...
          aria-hidden="true"
          loading="lazy"
          className="hover-media pointer-events-none absolute inset-0 -z-20 h-full w-full scale-105 object-cover opacity-[0.14] saturate-125 transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-[1.075]"
        />
      )}
      ...
      <div className="grid min-h-[270px] gap-6 p-6 sm:grid-cols-[1fr_150px] sm:items-center sm:p-7">
        <div className="min-w-0">
          ...
          <h3 className="font-serif text-3xl font-bold leading-[1.02] text-white transition-colors duration-300 group-hover:text-[var(--track-accent)] sm:text-4xl">{track.title}</h3>
          ...
          <Link to={`/music/${track.id}`} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-full border border-white/10 bg-white/[0.025] px-4 text-xs font-bold text-white/58 transition-[border-color,background-color,color] duration-300 hover:border-white/25 hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">
            Открыть анонс <ArrowRight size={15} className="transition-transform duration-300 group-hover:translate-x-1" />
          </Link>
        </div>
        ...
        {track.coverUrl ? (
          <Link to={`/music/${track.id}`} aria-label={`Открыть анонс «${track.title}»`} className="relative mx-auto hidden aspect-square w-full max-w-[150px] overflow-hidden rounded-[1.45rem] border border-white/12 bg-black shadow-[0_22px_65px_rgba(0,0,0,0.52)] sm:block">
            <img src={asset(track.coverUrl)} alt={`Предварительная обложка «${track.title}»`} loading="lazy" className="hover-media h-full w-full object-cover opacity-90 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-[1.035]" />
