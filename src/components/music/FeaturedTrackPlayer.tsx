...
const time = (value: number) => `${Math.floor((value || 0) / 60)}:${Math.floor((value || 0) % 60).toString().padStart(2, '0')}`;
...
export default function FeaturedTrackPlayer({ track, compact = false }: { track: MusicTrack; compact?: boolean }) {
  ...
  const waveform = useMemo(() => track.waveform?.length ? track.waveform : Array.from({ length: 80 }, (_, i) => 0.28 + ((i * 17) % 65) / 100), [track.waveform]);
  ...
  useEffect(() => {
    ...
    const onPlay = () => { setPlaying(true); window.dispatchEvent(new CustomEvent('tlp-audio-playing', { detail: track.id })); };
    const onPause = () => setPlaying(false);
    const onEnd = () => { setPlaying(false); setCurrent(0); };
    const onOther = (event: Event) => { if ((event as CustomEvent<string>).detail !== track.id) audio.pause(); };
        ...
        <div className="min-w-0">
          ...
          <div className="mt-7">
            <div className="relative flex h-20 items-center gap-[2px] overflow-hidden rounded-2xl border border-cyan-400/10 bg-black/25 px-3">
              {waveform.map((peak, index) => <span key={index} aria-hidden="true" className={`min-w-[2px] flex-1 rounded-full ${index / waveform.length <= progress ? 'bg-luxury-gold' : 'bg-cyan-100/16'}`} style={{ height: `${Math.max(10, peak * 92)}%` }} />)}
              <input type="range" min={0} max={duration || 1} step="0.1" value={Math.min(current, duration || 1)} onChange={(event) => { const next = Number(event.target.value); setCurrent(next); if (audioRef.current) audioRef.current.currentTime = next; }} aria-label="Позиция воспроизведения" className="absolute inset-0 h-full w-full cursor-pointer opacity-0" />
            </div>
            <div className="mt-2 flex justify-between text-xs tabular-nums text-cyan-100/38"><span>{time(current)}</span><span>{time(duration || track.durationSeconds || 0)}</span></div>
          </div>
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <button type="button" onClick={toggle} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-luxury-gold px-6 text-sm font-bold text-black">{playing ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />} {playing ? 'Пауза' : 'Слушать'}</button>
            <button type="button" onClick={() => setMuted((value) => !value)} aria-label={muted ? 'Включить звук' : 'Выключить звук'} className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-cyan-400/15 text-cyan-100/60">{muted ? <VolumeX size={18} /> : <Volume2 size={18} />}</button>
            ...
            <button type="button" onClick={share} className="inline-flex min-h-12 items-center gap-2 rounded-full border border-cyan-400/15 px-4 text-xs font-bold text-cyan-100/60"><Share2 size={16} /> {shareLabel}</button>
            {compact && <Link to={`/music/${track.id}`} className="inline-flex min-h-12 items-center gap-2 rounded-full border border-luxury-gold/20 px-4 text-xs font-bold text-luxury-gold">Страница релиза <ExternalLink size={15} /></Link>}
          </div>
