    ...
    <AnimatePresence>
      {visible && currentTrack && (
        <motion.aside
          ...
          className="global-audio-mini theme-dark-island"
          aria-label="Текущий музыкальный релиз"
          style={getTrackThemeStyle(currentTrack)}
        >
          <div
            className="audio-mini-surface relative isolate overflow-hidden rounded-[1.45rem] border border-white/[0.11] shadow-[0_24px_80px_rgba(0,0,0,0.62)] backdrop-blur-2xl"
          ...
          >
            ...
            <div className="seek-focus-shell absolute inset-x-0 top-0 z-20 h-2">
              ...
              <span
                ...
                className="seek-focus-indicator pointer-events-none absolute inset-x-1 -top-1 h-4 rounded-full border"
              />
            </div>

            <div className="flex min-w-0 items-center gap-3 p-2.5 pr-3 sm:gap-4 sm:p-3 sm:pr-4">
              <Link to={`/music/${currentTrack.id}`} className="audio-cover-surface relative h-14 w-14 flex-none overflow-hidden rounded-2xl border border-white/10 sm:h-16 sm:w-16" aria-label={`Открыть релиз «${currentTrack.title}»`}>
                {currentTrack.coverUrl && <ResilientImage src={currentTrack.coverUrl} alt="" sizes="64px" className="h-full w-full object-cover" />}
                <span className={`absolute inset-0 transition ${playing ? 'bg-transparent' : 'audio-cover-dim'}`} />
              </Link>

              <div className="min-w-0 flex-1">
                <div className="audio-status mb-1 flex min-w-0 items-center gap-2 text-[9px] font-bold uppercase tracking-[0.15em]" aria-live="polite" title={status === 'error' ? failure?.message : undefined}>
                ...
                <Link to={`/music/${currentTrack.id}`} className="audio-title block truncate font-serif text-lg font-bold leading-none transition hover:text-[var(--track-accent)] sm:text-xl">{currentTrack.title}</Link>
                <div className="audio-meta mt-1 flex items-center gap-2 text-[11px]">
                  <span className="truncate">{currentTrack.poet}</span>
                  <span className="audio-meta-subtle">·</span>
              ...
              <div className="flex flex-none items-center gap-1 sm:gap-1.5">
                <button type="button" onClick={() => { void playPrevious(); }} aria-label="Предыдущий релиз" className="audio-control hidden h-10 w-10 items-center justify-center rounded-full transition sm:inline-flex"><ChevronLeft size={18} /></button>
                <button
                  type="button"
                ...
                <button type="button" onClick={() => { void playNext(); }} aria-label="Следующий релиз" className="audio-control hidden h-10 w-10 items-center justify-center rounded-full transition sm:inline-flex"><ChevronRight size={18} /></button>
                <button type="button" onClick={() => openImmersive()} aria-label="Открыть режим погружения" className="audio-control inline-flex h-10 w-10 items-center justify-center rounded-full transition"><Maximize2 size={17} /></button>
                <button type="button" onClick={closePlayer} aria-label="Закрыть плеер" className="audio-control-close inline-flex h-10 w-10 items-center justify-center rounded-full transition"><X size={17} /></button>
              </div>
            </div>
          </div>
        </motion.aside>
