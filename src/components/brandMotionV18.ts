...
export function createBrandMotionController(node: HTMLElement): BrandMotionController {
  ...
  const write = () => {
    ...
    style.setProperty('--brand-root-y', px(-0.78 * figureWake));
    style.setProperty('--brand-root-scale', number(1 + 0.022 * figureWake));

    style.setProperty('--brand-far-x', px(-x * 3.15 * auraWake));
    style.setProperty('--brand-far-y', px((-y * 2 - 0.5) * auraWake));
    style.setProperty('--brand-far-scale', number(1 + 0.042 * auraWake));

    style.setProperty('--brand-energy-x', px(x * 3.65 * auraWake));
    style.setProperty('--brand-energy-y', px((y * 2.35 - 0.36) * auraWake));
    style.setProperty('--brand-energy-r', degrees(x * 0.82 * auraWake));
    style.setProperty('--brand-energy-scale', number(1 + 0.022 * auraWake));

    style.setProperty('--brand-figure-x', px(x * 0.18 * figureWake));
    style.setProperty('--brand-figure-y', px((y * 0.13 - 0.14) * figureWake));

    style.setProperty('--brand-folds-x', px(x * 1.1 * figureWake));
    style.setProperty('--brand-folds-y', px((y * 0.68 + 0.18) * figureWake));
    ...
    style.setProperty('--brand-hood-x', px(x * 1.28 * figureWake));
    style.setProperty('--brand-hood-y', px((y * 0.82 - 0.24) * figureWake));
    style.setProperty('--brand-hood-scale', number(1 + 0.012 * figureWake));

    style.setProperty('--brand-hood-layers-x', px(x * 2.02 * detailWake));
    style.setProperty('--brand-hood-layers-y', px((y * 1.24 - 0.34) * detailWake));
    style.setProperty('--brand-hood-layers-scale', number(1 + 0.02 * detailWake));

    style.setProperty('--brand-face-x', px(-x * 0.72 * detailWake));
    style.setProperty('--brand-face-y', px((-y * 0.4 + 0.16) * detailWake));
    style.setProperty('--brand-face-scale', number(1 - 0.022 * detailWake));

    style.setProperty('--brand-collar-x', px(x * 1.38 * figureWake));
    style.setProperty('--brand-collar-y', px((y * 0.9 + 0.16) * figureWake));
    ...
    style.setProperty('--brand-rim-x', px(x * 2.55 * detailWake));
    style.setProperty('--brand-rim-y', px((y * 1.5 - 0.2) * detailWake));
    style.setProperty('--brand-texture-x', px(x * 0.86 * detailWake));
    style.setProperty('--brand-texture-y', px(y * 0.54 * detailWake));
