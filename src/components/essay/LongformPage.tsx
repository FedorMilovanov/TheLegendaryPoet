...
import ShareLine from '../ui/ShareLine';
import ReadingProgress from '../articles/ReadingProgress';
import CommunityPanel from '../community/CommunityPanel';
import { articleRatingDimensions } from '../../data/ratingDimensions';
import ArticleRenderer, { getEssayToc } from './ArticleRenderer';
...
export default function LongformPage({
  essay,
  seriesEntries = [],
  seriesHref = (entry) => `/essays/${entry.slug}`,
  afterArticle,
}: LongformPageProps) {
  const articleRef = useRef<HTMLElement>(null);
  const toc = getEssayToc(essay.blocks);
  const poet = essay.poetId ? poets.find((entry) => entry.id === essay.poetId) : undefined;
  const orderedSeries = [...seriesEntries].sort(
    (a, b) => (a.series?.part ?? 0) - (b.series?.part ?? 0),
  );
  const previous = orderedSeries.find(
    (entry) => entry.series?.part === (essay.series?.part ?? 0) - 1,
  );
  const next = orderedSeries.find(
    (entry) => entry.series?.part === (essay.series?.part ?? 0) + 1,
  );
  const sourceCount = essay.sources?.length ?? 0;
  const primarySourceCount =
    essay.sources?.filter((source) => source.kind === 'primary').length ?? 0;
  const hasRail = toc.length > 0 || sourceCount > 0 || Boolean(poet);

  return (
    <div className="min-h-screen bg-[#050505] pt-28 pb-24 text-white">
      <ReadingProgress />
      {toc.length > 0 && <SectionChip toc={toc} />}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div
          className={`mx-auto mt-14 max-w-6xl gap-10 ${
            hasRail ? 'grid lg:grid-cols-[220px_1fr]' : 'flex justify-center'
          }`}
        >
          {hasRail && (
              ...
              <div className="sticky top-28">
                {toc.length > 0 && (
                  <nav className="rounded-2xl border border-luxury-gold/10 bg-[#0a0a0a]/70 p-5 backdrop-blur-xl">
                    <div className="mb-3 text-[10px] font-bold uppercase tracking-[0.18em] text-luxury-gold/70">
                      Оглавление
                    </div>
                    <ul className="space-y-1">
                      {toc.map((section) => (
                        <li key={section.anchor}>
                          ...
                          >
                            <span className="font-serif text-[12px] font-semibold tabular-nums text-luxury-gold/55">
          ...
          <article ref={articleRef} className="min-w-0 w-full max-w-3xl">
            ...
            {orderedSeries.length > 1 && (
              <nav
                ...
                className="mt-14 rounded-[2rem] border border-luxury-gold/10 bg-[#0a0a0a]/60 p-6 md:p-8"
              >
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {previous ? (
                    <Link
                      to={seriesHref(previous)}
                      className="group flex min-h-24 items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.02] p-4 text-left transition-[transform,border-color,background-color] duration-300 hover:-translate-y-0.5 hover:border-luxury-gold/25 hover:bg-luxury-gold/[0.03]"
                    >
                      <ArrowLeft
                        size={16}
                        className="shrink-0 text-luxury-gold/55 transition group-hover:-translate-x-1"
                      />
                      <span>
                        <span className="block text-[9px] uppercase tracking-[0.16em] text-luxury-gray-light/40">
                          Предыдущая часть
                        </span>
                        <span className="mt-1 block font-serif text-lg text-white/85">
                          {previous.title}
                        </span>
                      </span>
                    </Link>
                  ...
                  {next ? (
                    <Link
                      to={seriesHref(next)}
                      className="group flex min-h-24 items-center justify-between gap-3 rounded-2xl border border-white/8 bg-white/[0.02] p-4 text-right transition-[transform,border-color,background-color] duration-300 hover:-translate-y-0.5 hover:border-luxury-gold/25 hover:bg-luxury-gold/[0.03]"
                    >
                      <span>
                        <span className="block text-[9px] uppercase tracking-[0.16em] text-luxury-gray-light/40">
                          Следующая часть
