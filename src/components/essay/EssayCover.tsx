...
/**
 * Shared essay cover surface. Archive covers resolve through the generated
 * AVIF/WebP manifest; local editorial covers retain the conventional 640/full
 * sibling fallback. ResilientImage owns honest load/failure state without
 * replacing the specialised responsive-media pipeline.
 */
...
export default function EssayCover({
  ...
  const imageLoaded = imageState === 'loaded' || imageState === 'fallback';
  ...
  return (
    ...
    >
      <picture className={`block h-full w-full leading-none transition-opacity duration-500 ${imageFailed ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <ResilientImage
          ...
          className={`essay-cover-image block h-full w-full object-cover ${imageLoaded ? 'is-loaded' : ''} ${imgClassName}`}
      ...
      {imageFailed && (
        <div className="absolute inset-0 flex h-full w-full items-center justify-center" data-testid="essay-cover-fallback">
