...
import { ArrowLeft, ArrowRight, BookOpen, ExternalLink, Layers3 } from 'lucide-react';
            ...
            {seriesEntries.length > 1 && (
              <nav aria-label="Навигация по серии" className="mt-14 rounded-[2rem] border border-luxury-gold/10 bg-[#0a0a0a]/60 p-6 md:p-8">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {previous ? (
                    <Link to={`/essays/${previous.slug}`} className="group flex min-h-24 items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.02] p-4 text-left transition hover:border-luxury-gold/25 hover:bg-luxury-gold/[0.03]">
                  ...
                  {next ? (
                    <Link to={`/essays/${next.slug}`} className="group flex min-h-24 items-center justify-between gap-3 rounded-2xl border border-white/8 bg-white/[0.02] p-4 text-right transition hover:border-luxury-gold/25 hover:bg-luxury-gold/[0.03]">
                      <span>
            ...
            {essay.sources && essay.sources.length > 0 && (
              <section className="mt-14 rounded-[2rem] border border-luxury-gold/10 bg-[#0a0a0a]/60 p-8">
                ...
                <ul className="space-y-3">
                  {essay.sources.map((source, index) => (
                    <li key={index} className="text-sm leading-relaxed text-luxury-gray-light/80">
                      {source.url ? (
                        <a href={source.url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-9 items-start gap-1.5 py-1 text-luxury-gray-light/80 transition-colors hover:text-luxury-gold">
                          {source.title} <ExternalLink size={12} className="mt-1 shrink-0 opacity-60" />
                        </a>
                      ) : (
                        source.title
                      )}
