...
import { ArrowLeft, ChevronDown, Disc3, FileCheck2, Fingerprint, Headphones, Quote, Sparkles } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#050505] pb-24 text-white">
      <section className="relative min-h-[68vh] overflow-hidden pt-24">
        {track.wideCoverUrl && (
          <img
            ...
            alt=""
            aria-hidden="true"
            className="absolute inset-0 h-full w-full scale-[1.025] object-cover object-center opacity-62 saturate-[1.08]"
          />
        )}
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(5,5,5,0.9)_0%,rgba(5,5,5,0.46)_48%,rgba(5,5,5,0.38)_74%,rgba(5,5,5,0.68)_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_35%,rgba(0,212,255,0.1),transparent_28%),radial-gradient(circle_at_24%_22%,rgba(212,175,55,0.1),transparent_30%)]" />
        ...
        <div className="relative z-10 mx-auto grid min-h-[68vh] max-w-7xl items-end gap-10 px-4 pb-20 sm:px-6 lg:grid-cols-[1fr_310px] lg:px-8">
          <div className="max-w-4xl">
            ...
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full border border-luxury-gold/28 bg-black/34 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-luxury-gold backdrop-blur-xl"><Disc3 size={14} /> Официальный релиз · {track.releaseYear}</span>
              <span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/14 bg-black/28 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.15em] text-cyan-100/55 backdrop-blur-xl"><Headphones size={14} /> {track.duration} · 44.1 kHz</span>
            </div>
          ...
          {track.coverUrl && (
            <div className="relative mx-auto hidden w-full max-w-[310px] lg:block">
              <div className="absolute -inset-8 rounded-full bg-cyan-300/[0.1] blur-3xl" />
              <div className="relative overflow-hidden rounded-[2rem] border border-white/15 bg-black shadow-[0_35px_95px_rgba(0,0,0,0.65)]">
                <img src={asset(track.coverUrl)} alt={`Обложка трека «${track.title}»`} className="aspect-square w-full object-cover" />
      ...
      <div className="relative z-20 mx-auto -mt-8 max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.86fr]">
          <div className="relative overflow-hidden rounded-[2rem] border border-cyan-400/12 bg-[#071018]/72 p-6 shadow-[0_22px_70px_rgba(0,0,0,0.24)] sm:p-8">
            <div className="pointer-events-none absolute right-[-4rem] top-[-5rem] h-52 w-52 rounded-full bg-cyan-300/[0.055] blur-3xl" />
            <div className="relative mb-5 flex items-center gap-2 text-cyan-300"><FileCheck2 size={19} /><h2 className="font-serif text-2xl font-bold text-white">Паспорт релиза</h2></div>
            <div className="relative space-y-3">
              {track.credits?.map((credit, index) => (
                <div key={credit} className="grid grid-cols-[28px_1fr] items-center gap-3 rounded-2xl border border-cyan-400/[0.08] bg-black/22 px-4 py-3 text-sm text-cyan-100/56">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full border border-cyan-300/10 text-[10px] font-bold text-cyan-300/55">{String(index + 1).padStart(2, '0')}</span>
                  <span>{credit}</span>
                </div>
              ))}
            </div>
            {track.poetId && <Link to={`/poets/${track.poetId}`} className="relative mt-6 inline-flex min-h-11 items-center rounded-full border border-luxury-gold/22 bg-luxury-gold/[0.035] px-5 text-xs font-bold uppercase tracking-[0.14em] text-luxury-gold transition hover:bg-luxury-gold/[0.08]">Страница поэта</Link>}
          </div>

          <div className="relative overflow-hidden rounded-[2rem] border border-luxury-gold/14 bg-[linear-gradient(145deg,rgba(212,175,55,0.06),rgba(0,212,255,0.025))] p-6 shadow-[0_22px_70px_rgba(0,0,0,0.22)] sm:p-8">
            <div className="pointer-events-none absolute bottom-[-5rem] right-[-5rem] h-52 w-52 rounded-full bg-luxury-gold/[0.045] blur-3xl" />
            <div className="relative mb-5 flex items-center gap-2 text-luxury-gold"><Quote size={19} /><h2 className="font-serif text-2xl font-bold text-white">О публикации</h2></div>
            <p className="relative text-sm leading-relaxed text-cyan-100/57">{track.description}</p>

            <details className="group relative mt-6 rounded-2xl border border-white/[0.08] bg-black/22 transition open:border-cyan-300/12">
        ...
        {relatedTracks.length > 0 && (
          <section className="mt-20 border-t border-white/[0.07] pt-12" aria-labelledby="related-releases-title">
            <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
              <div>
                <div className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-300/52">Продолжить слушать</div>
              ...
              <Link to="/music" className="inline-flex min-h-11 items-center gap-2 rounded-full border border-white/10 px-4 text-xs font-bold text-cyan-100/48 transition hover:border-cyan-300/22 hover:text-cyan-200">Весь аудиоархив</Link>
            </div>
