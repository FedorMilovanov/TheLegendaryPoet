...
const sections = [
  ...
  {
    title: 'Нравственная ясность без упрощения человека',
    text: 'Доказанные богохульство, ложь, жестокость, прелюбодеяние, пьянство, зависимость и саморазрушение не романтизируются и не прячутся за красивыми эвфемизмами. При этом поступок не даёт права выдумывать мотив, медицинский диагноз или степень вины, которых источники не показывают. Сострадание к человеку не является оправданием зла, а нравственная оценка не отменяет человеческого достоинства.',
  },
  {
    title: 'Благовестие без морализаторства',
    text: 'Библейская рефлексия появляется не в каждой статье, а только там, где сам материал поднимает вопросы Бога, греха, покаяния, смерти и надежды. Поэтический религиозный образ не выдаётся за исповедание веры, а неизвестный последний миг — за доказанную историческую версию. Евангелие не используется как декоративная мораль: оно предлагается как серьёзный ответ на уже поставленный текстом вопрос.',
  },
  {
    title: 'Мифы, мемуары и степень уверенности',
    text: 'Позднее яркое воспоминание не получает вес синхронного письма или документа только потому, что оно хорошо звучит. Популярные легенды получают прослеживаемый источник, статус и безопасную формулировку; неподтверждённая реплика, диагноз или причинная схема не переходят в текст как установленный факт.',
  },
  {
    title: 'Исправления',
...
export default function EditorialPolicyPage() {
  ...
  const description = 'Как THE LEGENDARY POET проверяет факты, оформляет источники, обозначает реконструкции и отделяет документальную работу от нравственной и духовной интерпретации.';
    ...
    <div className="min-h-screen bg-[#050505] pb-24 pt-28 text-white">
      <main className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <p className="mt-10 text-sm text-cyan-100/35">
          Дата публикации политики: 28 июля 2026 года. Существенное обновление: 2 августа 2026 года. См. также <Link to="/privacy" className="text-cyan-300/75 hover:text-cyan-200">политику конфиденциальности</Link>.
        </p>
      </main>
    </div>
  );
